...
public class FListItemDelete extends LinearLayout {
	...
	public FListItemDelete(Context context) {
		this(context, null);
	}

	public FListItemDelete(Context context, AttributeSet attrs) {
		super(context, attrs);
		init(context);
	}
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_UP:
			...
			if (Math.abs(x - downX) < itemClickMin) {// 这里根据点击距离来判断是否是itemClick
				FPlanAdapter.ItemDeleteReset();
				return false;
			}
			if (deltaX > 0) {
				if (scroll > back_width / 4) {
					scrollTo(back_width, 0);
					FPlanAdapter.itemDelete = this;
				} else {
					reSet();
				}
			} else {
